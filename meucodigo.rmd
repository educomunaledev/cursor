...
br_cov = cobertura_vacinal %>%
 ...
 ggplot() +
  ...
  gghighlight(name %in% c("Brazil", "Colombia", "Canada"), 
...
#GGHIHGLIGHT ajuda a filtrar as linhas de um gráfico complexo
#scale_color_manual() ele cria um vetor e vc consegue alterar as cores manualmente
br_cov
```
...
### GERANDO NOVOS GRÁFICOS SOZINHO
...
fr_jp_cov = cobertura_vacinal %>%
 filter(year >= 2012 & year <= 2022,
        name %in% c("France", "Japan")) %>%
 ggplot() +
  aes(x = year, 
      y = coverage,
      colour = name) +
...
mortes = mortes_doencas %>%
 ...
 ggplot() +
  ...
  #highlight
  gghighlight(name %in% c("France", "Japan"),
...
```{r}
mortes = mortes_doencas %>%
 filter( year == 2022, death_disease == 'Meningitis') %>%
  select(name, year, total_deaths, death_rate_100thousand, death_disease) %>% 
  distinct() %>%
  drop_na(total_deaths) %>% 
  ggplot() +
...
```{r}
dados = mortes_doencas %>% 
  
filter( year == 2022, death_disease == 'Meningitis') %>%

  select(name, year, total_deaths, continent, death_disease) %>% 
  distinct() %>%
  drop_na(total_deaths)

dados$name <- reorder(dados$name, dados$total_deaths)


ggplot(data = dados, aes(x = name, y = total_deaths, fill = continent)) +
  geom_col() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +  
  labs(title = "Mortes por Meningite em 2022", x = "País", y = "Total de Mortes") 
```

### Pergunta 1: Quais países apresentam a maior taixa de mortalidade para a doença de Meningite (Meningitis) no ano de 2022?
...
### Pergunta 2: Quais países da europa e ásia central apresentaram a menor taixa de vacinação contra a Poliomelite no ano de 2022?

```{r}
dados2 = cobertura_vacinal %>%
  
filter(year == 2022, vaccine == 'IPV1', region_complete == "Europe and Central Asia") %>% 
  
  select(name, year, continent, coverage, vaccine) %>%
  distinct() %>%
  drop_na(coverage)

dados2$name <- reorder (dados2$name, dados2$coverage)

ggplot(data = dados2, aes(x = name, y = coverage, fill = continent)) +
  geom_col() +
  theme_minimal() +
  theme (axis.text.x = element_text(angle = 90, hjust = 1),
         plot.title = element_text(hjust = 0.5)) +
  labs(title = "Cobertura Vacinal Poliomelite em 2022 na Europa e na Ásia Central", x = "Páis", y = "Vacinação")
```

### Pergunta 3: Paises latino americanos apresentam uma taixa alta de mortalidade para a doença de Tuberculose? 

```{r}
dados_tubercolose = mortes_doencas %>% 
  
filter(death_disease == 'Tuberculosis', region_complete == "Latin America and Caribbean") %>%
...
ggplot(data = dados_tubercolose, aes(x = name, y = death_disease, fill = continent)) +
  geom_col() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +  
  labs(title = "Mortes por Tuberculosis 2000 - 2023", x = "País", y = "Total de Mortes") 
```
...
dados_tuberculose <- mortes_doencas %>% 
  filter(year == 2022, death_disease == 'Tuberculosis') %>%
  select(name, year, total_deaths, continent, death_disease) %>% 
  distinct() %>%
  drop_na(total_deaths) %>% 
  mutate(name = fct_reorder(name, total_deaths))  
  


dados_tuberculose %>% 
  ggplot(aes(x = name, y = total_deaths, fill = continent)) +
  geom_col() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +  
  labs(title = "Mortes por Tuberculosis 2022", x = "País", y = "Total de Mortes")


```

### Pergunta 4: Porque o México apresenta uma taixa de mortalidade tão alta para Tuberculose? 

```{r}
library(dplyr)
library(ggplot2)

dados3 <- cobertura_vacinal %>%
  filter(year == 2022, name %in% c('Mexico', 'Brazil')) %>% 
  select(name, year, continent, coverage, vaccine) %>%
  distinct() %>%
  drop_na(coverage)

dados3$name <- reorder(dados3$name, dados3$coverage)

ggplot(data = dados3, aes(x = name, y = coverage, fill = name)) +
  geom_col(width = 0.3) +  # Definir a largura das barras como 0.7 (mais estreitas)
  scale_fill_manual(values = c('Mexico' = 'blue', 'Brazil' = 'green')) +  # Definir cores para cada país
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "Cobertura Vacinal: México, Brasil e Outros Países", x = "País", y = "Vacinação") +
...
dados3 <- cobertura_vacinal %>%
  filter(vaccine == 'BCG', continent == "Americas") %>%
  select(name, year, coverage) %>%
  distinct() %>%
  drop_na(coverage)

# Identificar países para agrupar como "Outros"
paises_outros <- dados3$name[!(dados3$name %in% c('Mexico', 'Brazil'))]

# Agrupar países como "Outros"
dados3 <- dados3 %>%
  mutate(name = ifelse(name %in% c('Mexico', 'Brazil'), name, 'Outros'))

# Agregar dados para "Outros"
dados3 <- dados3 %>%
  group_by(name, year) %>%
  summarise(coverage = mean(coverage)) %>%
  ungroup()

# Reordenar "Outros" para o final
dados3$name <- factor(dados3$name, levels = c('Mexico', 'Brazil', 'Outros'))

# Plotar o gráfico de barras
ggplot(data = dados3, aes(x = year, y = coverage, color = name)) +
  geom_line(size = 2) +
  scale_color_manual(values = c('Mexico' = 'blue', 'Brazil' = 'green', 'Outros' = 'gray')) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "Cobertura Vacinal: México, Brasil e Outros Países", x = "País", y = "Vacinação")

# Sempre que usar o gpt pedir para fazer como se fosse no tidyverse
...
dados3 <- cobertura_vacinal %>%
  ...
  distinct() %>%
  drop_na(coverage) %>% 
  mutate(name = if_else(name %in% c('Mexico', 'Brazil'), name, 'Outros')) %>%
  group_by(name, year) %>%
  summarise(coverage = mean(coverage)) %>%
  ungroup()
...
# Reordenar "Outros" para o final
dados3 <- dados3 %>%
  mutate(name = fct_relevel(factor(name), 'Mexico', 'Brazil', 'Outros'))
...
ggplot(data = dados3, aes(x = year, y = coverage, color = name)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "Cobertura Vacinal: México, Brasil e Outros Países", x = "Ano", y = "Cobertura")
