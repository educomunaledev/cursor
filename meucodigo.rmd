...
ggplot(data = dados, aes(x = name, y = death_disease, fill = continent)) +
  geom_col() +
  theme_minimal() +
...
dados_tuberculose$name <- reorder(dados_tuberculose$name, dados_tuberculose$total_deaths)

ggplot(data = dados_tuberculose, aes(x = name, y = total_deaths, fill = continent)) +
  geom_col() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +  
...
dados3 <- cobertura_vacinal %>%
  filter(name %in% c('Mexico', 'Brazil')) %>% 
  select(name, year, continent, coverage, vaccine) %>%
  distinct() %>%
  drop_na(coverage)
...
dados3 <- cobertura_vacinal %>%
  filter(continent == "Latin America and Caribbean") %>%
...
ggplot(data = dados3, aes(x = name, y = coverage, fill = name)) +
  geom_col() +
  scale_fill_manual(values = c('Mexico' = 'blue', 'Brazil' = 'green', 'Outros' = 'gray')) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "Cobertura Vacinal: México, Brasil e Outros Países", x = "País", y = "Vacinação")
